...
- **Unreadable characters are `?`.** If a character is printed but you cannot read it — blurred, under glare, faded, cut by a fold — write `?` in its place, one `?` per character: `Jan 2?, 2027`. Do not choose the likeliest digit. A guessed digit looks exactly like a read one, and nothing downstream can tell them apart. A date with any `?` is not a full date: leave its ISO slot `null`. This applies to any text slot, not only dates. Use `?` only where something is printed; a slot the page leaves empty is `null`.
- **Every row of the page's lists is a line item** — billed services, vaccinations, reminders, records — whether or not it looks clinical. A discount line, a technician fee, a diagnostic test, a dewormer: transcribe all of them. Deciding which rows matter is not your job. A reference number printed beside a field (an invoice or account number), a heading, a footer and a sentence of prose are not rows, however they are laid out. Number the rows in the order they appear on the page, starting at 1.
...
- `address_raw` is the address as one string. If it is printed over several lines, join the lines with a single space and add no punctuation the page does not print.
- The owner's address is always split, because the owner block has no `address_raw`. Street and number go in `address_line1`; a unit designator goes in `address_line2` exactly as printed (`#511`, `Apt 511`); city, state and postal code go in their own slots — even when the page prints them as one run-on line.
- When the page prints the same fact twice in two forms — a phone number in the header and again in a contact block — either printed form is a correct transcription. Use one of them as printed; do not reformat it.
